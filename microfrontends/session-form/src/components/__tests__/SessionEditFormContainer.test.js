...
describe('SessionEditFormContainer', () => {
  ...
  it('saves data', async () => {
    ...
    await wait(() => {
      ...
      expect(apiSessionPut).toHaveBeenCalledWith('', sessionMock.id, sessionMock);
  ...
  it('shows an error if data is not successfully saved', async () => {
    ...
    const { findByTestId, getByText } = render(
      <SessionEditFormContainer id="1" onError={onErrorMock} config={configMock} />
    );
