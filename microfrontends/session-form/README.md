...
1. Go to widget source (<MICROSERVICE_FOLDER>/ui/widgets/session/formWidget/>):
   1. Run `npm i`
...
- **session.form.createError** (fired when an error occurs during session creation)
- **session.form.create** (fired when the session is successfully created)
- **session.form.updateError** (fired when an error occurs during session update)
- **session.form.update** (fired when the session is successfully updated)
