...
import { apiSessionsGet, apiSessionsDelete } from 'api/sessions';
import { reducer, initialState } from 'state/session.reducer';
import { ADD_FILTER, UPDATE_FILTER, DELETE_FILTER, CLEAR_FILTERS } from 'state/filter.types';
import { DELETE, ERROR_FETCH, CLEAR_ERRORS, READ_ALL, CLEAR_ITEMS } from 'state/session.types';
...
class SessionTableContainer extends Component {
  constructor(props) {
    super(props);
  ...
  async fetchData() {
    ...
    const serviceUrl =
      ...
      config.systemParams.api['session-api'].url;
    const authenticated = keycloak.initialized && keycloak.authenticated;
    ...
    if (authenticated) {
      try {
        ...
        const { sessions, headers } = await apiSessionsGet(serviceUrl, requestParameters);
        const sessionCount = (headers && headers['X-Total-Count']) || 0;

        this.dispatch({
          type: READ_ALL,
          payload: {
            items: paginationMode === 'infinite-scroll' ? [...items, ...sessions] : sessions,
            count: sessionCount,
          },
        });
      } catch (err) {
  ...
  async handleDelete(item) {
    ...
    if (authenticated) {
      try {
        await apiSessionsDelete(serviceUrl, item.id);
        onDelete(item);
        ...
        <AuthenticatedView keycloak={keycloak}>
          ...
          <PaginationWrapper items={items} paginationMode={paginationMode} count={count}>
            <div className={classes.tableWrapper}>
              <SessionTable items={items} onSelect={onSelect} Actions={Actions} />
            </div>
          </PaginationWrapper>
        </AuthenticatedView>
        <Notification
...
SessionTableContainer.propTypes = {
...
SessionTableContainer.defaultProps = {
  onAdd: () => {},
  onDelete: () => {},
  onError: () => {},
  onSelect: () => {},
...
export default withKeycloak(
  withStyles(styles)(
    withTranslation(undefined, { withRef: true })(withPagination(SessionTableContainer))
  )
);
