...
1. Go to widget source (<MICROSERVICE_FOLDER>/ui/widgets/session/tableWidget/>):
   1. Run `npm i`
...
- **session.table.error** (fired when an error occurs)
- **session.table.select** (fired when a table row is selected)
